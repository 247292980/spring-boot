...
(function() {
	...
	f.core = function(a) {
		...
		this.src = b.swfUrl;
		if (!(this.src && typeof this.src.toString === 'function' && this.src
				.toString().length > 0)) {
			f.raiseError(this, '25081843', 'param', '',
					'Missing required parameter swfUrl.');
			throw '#25081843 FusionCharts > Missing required parameter swfUrl.';
		}
		this.args = b;
...
(function() {
	...
	var m = {
		outputHelpers : {
			'text' : function(e) {
				var a = (e.sender.id || e.sender).toString();
				m.outputTo('#' + e.eventId + ' [' + a + '] fired "'
						+ e.eventType + '" event.')
			},
			'event' : function(e, a) {
				this.outputTo(e, a)
...
(function() {
	...
	f.addEventListener('Loaded', function(e) {
		...
		if (a === undefined || a === null
				|| typeof a.getExternalInterfaceMethods !== 'function') {
			return
		}
		var b = a.getExternalInterfaceMethods().split(','), i;
		for (i = 0; i < b.length; i += 1) {
			if (e.sender[b[i]] === undefined) {
				e.sender[b[i]] = j(b[i])
			}
		}
	});
	...
	var l = function(a) {
		if (a.success === false) {
			f.raiseError(f.core.items[a.id], '25081850', 'run',
					'::RendererManager', new Error(
			...
			.extend(
					{
						render : function(a) {
							if (window[this.id] !== undefined) {
								f
										.raiseError(
												...
												'.render',
												new Error(
														'#25081843:IECompatibility() Chart '
							...
							if (k(this.id, a)) {
								f.raiseError(new Error(
										"#03091502 FusionCharts.render() "
												+ new Error(
...
(function() {
	var j = FusionCharts([ 'private', 'DataHandlerManager' ]);
	if (j === undefined) {
		return
	}
	...
	j.ajax.error = function(a, x, b, c, e) {
		var d = {
			source : b,
			url : a,
		...
		j.raiseEvent('DataLoadError', d, c);
		if (typeof window.FC_DataLoadError === 'function') {
			window.FC_DataLoadError(c.id, d)
		}
	};
	var k = {}, dataStore = {}, xStore = {}, cache = {};
	j.policies.options.dataSource = [ 'dataSource', undefined ];
	j.policies.options.dataFormat = [ 'dataFormat', undefined ];
	j.core.addEventListener('Disposed', function(e) {
		delete cache[e.sender.id]
	});
	j.addDataHandler = function(b, c) {
		if (typeof b !== 'string' || k[b.toLowerCase()] !== undefined) {
			j.raiseError(j.core, '03091606', 'param',
		...
		d['set' + b + 'Url'] = function(a) {
			return this.setChartData(a, b + 'url')
		};
	...
	j.addEventListener('BeforeInitialize', function(a) {
		var b = a.sender;
		dataStore[b.id] = '';
		cache[b.id] = {};
	...
	j.addEventListener('BeforeDispose', function(e) {
	...
	j
			.extend(
					{
						setChartDataUrl : function(a, b, c) {
							if (typeof b !== 'string') {
								j.raiseError(j.core, '03091609', 'param',
										'.setChartDataUrl', new Error(
												'Invalid Data Format'));
								return
							}
							this.setChartData(a, /url$/i.test(b) ? b : b
									+ 'url', c)
						},
						setChartData : function(c, d, e, f) {
							if (d === undefined || d === null
									|| typeof d.toString !== 'function') {
								j.raiseError(j.core, '03091610', 'param',
							...
							if (/url$/i.test(d)) {
								...
								if (f !== true) {
									...
									if (xStore[this.id]
											&& typeof xStore[this.id].abort === 'function') {
										if (xStore[this.id].readyState
												&& xStore[this.id].readyState !== 0) {
											xStore[this.id].abort()
										}
									}
									xStore[this.id] = j.ajax
											.get(
							...
							this.options.dataFormat = d;
							var i = k[g], parseResult, eventArgs, cancelDUFlag = false;
							if (typeof i === 'undefined') {
								j.raiseError(j.core, '03091611', 'param',
										'.setChartData', new Error(
												'Data Format not recognized'));
								return
							}
							parseResult = i.encode(c, this);
							parseResult.format = g;
							eventArgs = {
								dataFormat : g,
								dataSource : c,
							...
							j.raiseEvent('BeforeDataUpdate', eventArgs, this);
							delete eventArgs.cancelDataUpdate;
							if (cancelDUFlag === true) {
								j.raiseEvent('DataUpdateCancelled', eventArgs,
										this);
								return
							}
							...
							cache[this.id] = {};
							if (e !== true) {
								j.renderer.update(this, parseResult)
							}
							j.raiseEvent('DataUpdated', eventArgs, this)
						},
						getChartData : function(a, b) {
							var c, parseResult;
							if (a === undefined
									|| typeof a.toString !== 'function'
									|| (c = k[a = a.toString().toLowerCase()]) === undefined) {
								j
										.raiseError(
												this,
												'25081543',
			...
			.extend(
					{
						transcodeData : function(a, b, c, d) {
							if (!b
									...
									|| k[(c = c.toString().toLowerCase())] === undefined
									|| k[(b = b.toString().toLowerCase())] === undefined) {
								j
										.raiseError(
												this,
												'14090217',
							...
							var e = k[b].encode(a, this), l2 = k[c].decode(
									e.data, this);
...
(function() {
	...
	window.getChartFromId = function(a) {
		return f.core.items[a] === undefined ? f.swfobject.getObjectById(a)
				: f.core.items[a]
	};
	var m = function(a, b) {
...
(function() {
	...
	window.__fusioncharts_vars = function(a, b) {
		...
		if (typeof b === 'object') {
			...
			if (b.dataURL !== undefined || b.dataXML !== undefined) {
	...
	var E = {
		...
		render : function(a, b) {
			...
			c.dataXML = window.encodeURIComponent(this.flashVars.dataXML || '');
			c.dataURL = C(this.flashVars.dataURL || '');
...
(function() {
	var J, FCC = {}, baseURL = 'JSClass/', covertToFCC, renderArray = [], FCCready = false, i, scriptTags, jsConf = {};
	J = FusionCharts([ 'private', 'Canvas_Renderer' ]);
	if (J === undefined) {
		return
	}
	function noEffect() {
	...
	FCC.init = function() {
		var m = {
			...
			method : {
				getExternalInterfaceMethods : function(a) {
					return m.commonMethodCSV
							+ (m.methodCSV[a || this.chartType()] || '')
				},
				print : function() {
					var a = this.id;
						...
						if (typeof a === 'object') {
							for ( var x in a) {
								if (x.toLowerCase() === 'exportformat'
										&& m.exportFormat[a[x].toLowerCase()]) {
									HCConf.type = m.exportFormat[a[x]
											.toLowerCase()]
				...
				getXML : function() {
					var a = this.id;
					return J.core.items[a].getXMLData()
				},
				signature : function() {
		...
		if (J.core.options && J.core.options.scriptBaseUri !== undefined) {
			baseURL = J.core.options.scriptBaseUri
		} else {
		...
		function createMsg(a, b, c) {
			var d = m.JSONconf.blankChart;
			d.chart.renderTo = a;
			d.title.text = n[a][b] ? n[a][b] : b;
			if (typeof c === 'string') {
				J.raiseEvent(c, {}, J.core.items[a])
			}
			return d
		}
		function showChartMessage(a) {
			var b = this.id, HCJson, FC = J.core.items[b];
			if (FC.ref) {
				n[b].msgTxt = a;
				FC.ref.FCC.destroy();
		...
		function drawOverlayButton(b) {
			if (b.show) {
				...
				c.onclick = (function(a) {
					return function() {
						J.raiseEvent('OverlayButtonClick', {}, a.FusionCharts)
					}
				}(this));
		...
		function getSeriesName(a) {
			return m.seriesName[a]
		}
		function convertColor(a, b, c, d) {
			...
			return 'rgba(' + parseInt(e.slice(0, 2), 16) + ','
					+ parseInt(e.slice(2, 4), 16) + ','
					+ parseInt(e.slice(4, 6), 16) + ',' + b + ')'
		}
		var o = function(x, y, a, b, c) {
			x = x ? x : 0;
			y = y ? y : 0;
		...
		var q = function(x, y, a, b, c, d, e, f) {
			var g = {};
			g.linearGradient = o(x, y, a, b, c);
			g.stops = p(d, e, f);
			if ('\v' === 'v') {
		...
		var A = function(a, b) {
			...
			if ((a === 2 && this.link) || (a === 1 && this.options.chart.link)) {
				...
				case 'j':
					...
					break;
				case 'p':
					w = window.open(arr[2], arr[1].match(/[^,]+/i), arr[1]
							.replace(/[^,]+,/i, ''));
					if (window.focus) {
						w.focus()
					}
				...
				case 'newchart':
					...
					if (d === 'URL') {
						...
						chartJson = J.core.items[chartId]
					...
					J.raiseEvent('LinkedChartInvoked', {
						linkType : d,
						data : data
					}, sender);
					break;
		...
		var D = function(a, b) {
			var c = this.width, height = this.height, chart = t(this.src), id = this.id, jsonObj, container, dataComp, HCJson;
			removeChart(id, b);
		...
		var E = function(a, b) {
			...
			var c = t(b.src);
			var d = m.method.getExternalInterfaceMethods(c).split(','), i;
			for (i = 0; i < d.length; i += 1) {
				if (typeof a[d[i]] === 'function') {
					continue
				}
				if (m.method[d[i]] === undefined) {
					a[d[i]] = noEffect
				} else {
					a[d[i]] = m.method[d[i]]
				}
		...
		function createChart(a, b) {
			...
			d.getExternalInterfaceMethods = m.method.getExternalInterfaceMethods;
			E(d, this);
			if (typeof a === 'function') {
				try {
					a({
					...
					(function(b) {
						return function() {
							...
							if (typeof __containerId !== 'undefined'
								...
								HCJson = D.call(b, __containerId);
								HCJson.chart.events.load = function() {
									J.raiseEvent('loaded', {}, b);
									J.raiseEvent('resized', {}, b)
								};
		...
		var H = function(a) {
			return {
				chart : {
					...
					events : {
						click : function() {
							A.call(this, 1, a.chart.unescapelinks)
						}
					},
					margin : [ 15, 15, 50, 60 ],
					borderRadius : 0,
				...
				plotOptions : {
					series : {
						...
						point : {
							events : {
								click : function() {
									A.call(this, 2, a.chart.unescapelinks)
								}
							}
						}
					},
				...
				yAxis : [ {
					...
					labels : {
						formatter : function() {
							return parseStr(u(this.value, a.chart, 1))
						}
					},
					plotBands : [],
					plotLines : []
					...
					labels : {
						enabled : false,
						formatter : function() {
							return parseStr(u(this.value, a.chart, 2))
						}
					},
					opposite : true
				} ],
		...
		function addVline(a, b, c, d, e) {
			a.xAxis.plotLines.push({
				color : convertColor(b, c, '#444444', 100),
				width : d ? d : 1,
				value : e
			})
		}
		function addPoint(a, b, c, d, e, f, g, h, i) {
			var x, y, isAbs = m.valueAbs[d], value, seriesType = a.type
					|| f.chart.defaultSeriesType, dataseperator;
			if (b instanceof Array) {
					...
					if (typeof b[y] === 'object') {
						if (!b[y].vline) {
							...
							a.data
									.push({
										y : (isAbs ? Math
												.abs(parseFloat(b[y].value
														|| b[y].y))
												: parseFloat(b[y].value
														|| b[y].y))
												|| '',
										x : (isAbs ? Math
										...
										z : (isAbs ? Math
												...
												: parseFloat(b[y].z))
												|| undefined,
										color : b[y].color || c < 2 ? convertColor(
												b[y].color || g(y),
												b[y].alpha || 100)
												: undefined,
										...
										marker : (seriesType === 'area' || seriesType === 'line') ? {
						...
						for (x = 0; x < value.length; x += 1) {
							a.data.push({
								y : value[x] ? (isAbs ? Math
										.abs(parseFloat(value[x]))
										: parseFloat(value[x])) : ''
							})
						}
					}
				...
				for (x = 0; x < b.dataset.length; x += 1) {
					...
					if (b.dataset[x].dataset
						...
						for (y = 0; y < b.dataset[x].dataset.length; y += 1) {
							newSeries = {
								FCtype : c,
								type : b.dataset[x].dataset[y].renderas ? m.combi[b.dataset[x].dataset[y].renderas
										.toLowerCase()]
										: undefined,
		...
		function addData(c, d, e, f, g, h, i) {
			var a = [], x, y, z, count_set, getScolor, pareto;
			getScolor = function(a) {
				var b;
				b = (c.chart.palettecolors) ? ((typeof b === 'string') ? c.chart.palettecolors
						.split(',')
			...
			case 1.5:
				if (d == 1.5) {
					...
					f.chart.alignTicks = false;
					f.yAxis[1].alternateGridColor = null;
					f.yAxis[1].gridLineWidth = 0
				}
					...
					if (c.dataset) {
						...
						for (x = 0; x < c.dataset.length; x += 1) {
							z = createSeries(
									...
									c.dataset[x].renderas ? m.combi[c.dataset[x].renderas
											.toLowerCase()]
			...
			case 7.5:
				...
				if (c.dataset) {
					...
					for (x = 0; x < c.dataset.length; x += 1) {
						...
						z.marker = {
							symbol : (d == 7.5) ? 'circle' : v(parseInt(
				...
				for (z = 0; z < a.series[y].data.length; z += 1) {
					...
					if (catCountFlag) {
						...
						if (b.chart.showsum != '0') {
							a.labels.items.push(createText(u(arr[z], b.chart),
									'-14', temp))
						}
					}
		...
		function covertToFCC(a, b, c, d, e, f) {
			...
			if (c.chart.drawanchors == '0') {
				g.plotOptions.area.marker.enabled = g.plotOptions.line.marker.enabled = false;
				g.tooltip.enabled = false
			}
			if (c.chart.anchorradius) {
				g.plotOptions.area.marker.radius = g.plotOptions.line.marker.radius = c.chart.anchorradius
			}
			g.plotOptions.line.marker.lineColor = convertColor(
					c.chart.anchorbordercolor, c.chart.anchoralpha, '767575',
					100);
			g.plotOptions.area.marker.lineColor = convertColor(
					c.chart.anchorbordercolor, c.chart.anchoralpha, '767575', 0);
			if (c.chart.anchorborderthickness) {
				g.plotOptions.area.marker.lineWidth = g.plotOptions.line.marker.lineWidth = c.chart.anchorborderthickness
			}
			g.plotOptions.line.marker.fillColor = convertColor(
					c.chart.anchorbgcolor, c.chart.anchorbgalpha, 'FFFFFF', 100);
			g.plotOptions.area.marker.fillColor = convertColor(
					c.chart.anchorbgcolor, c.chart.anchorbgalpha, 'FFFFFF', 0);
			if (c.chart.anchorsides) {
				g.plotOptions.area.marker.symbol = g.plotOptions.line.marker.symbol = v(parseInt(
						c.chart.anchorsides, 10))
			}
			g.plotOptions.line.lineWidth = parseInt(c.chart.linethickness, 10)
					|| (y === 1.5 ? 2 : 3);
			g.plotOptions.line.color = convertColor(c.chart.linecolor
					|| '767575', c.chart.linealpha || 100);
			g.plotOptions.area.color = convertColor(c.chart.plotfillcolor
					|| '767575', c.chart.plotfillalpha || 90);
			g.plotOptions.area.lineColor = convertColor(c.chart.plotbordercolor
					|| "333333", c.chart.plotborderalpha || 95);
			g.plotOptions.area.lineWidth = parseInt(
					c.chart.plotborderthickness, 10) || 1;
			g.yAxis[1].alternateGridColor = g.yAxis[0].alternateGridColor = convertColor(
					c.chart.alternatehgridcolor, c.chart.alternatehgridalpha,
					'EEEEEE', 50);
			g.yAxis[1].gridLineColor = g.yAxis[0].gridLineColor = convertColor(
					c.chart.divlinecolor, c.chart.divlinealpha, '717170', 40);
			g.yAxis[1].gridLineWidth = g.yAxis[0].gridLineWidth = c.chart.divlinethickness ? c.chart.divlinethickness
					: 1;
			if (c.chart.caption) {
			...
			g.tooltip.backgroundColor = convertColor(c.chart.tooltipbgcolor,
					c.chart.tooltipbgalpha, 'FFFFFF', 100);
			g.tooltip.borderColor = convertColor(c.chart.tooltipbordercolor,
					c.chart.tooltipborderalpha, '545454', 100);
			...
			g.chart.borderColor = convertColor(c.chart.bordercolor,
					c.chart.borderalpha, '767575', 50);
			g.chart.plotBorderColor = convertColor(c.chart.canvasbordercolor,
					c.chart.canvasborderalpha, '545454', 100);
			...
			g.plotOptions.series.borderColor = convertColor(
					c.chart.plotbordercolor, c.chart.plotborderalpha, '333333',
					95);
			...
			g.chart.backgroundColor = q(0, 0, e, f ? f : 400,
			...
			g.chart.plotBackgroundColor = (r(b) == 'pie') ? 'rgba(255,255,255,0)'
					: (q(
							h,
							plotY,
		...
		FCC.render = function(a, b) {
			var c, sender = this;
			if (FCC.isReady()) {
				c = D.call(this, a.id, true);
				c.chart.events.load = function() {
					J.raiseEvent('loaded', {}, sender);
					J.raiseEvent('dataloaded', {}, sender);
					J.raiseEvent('rendered', {}, sender);
					J.raiseEvent('drawcomplete', {}, sender);
					try {
						if (typeof window.FC_Loaded === 'function') {
							FC_Loaded(sender.id)
						}
						if (typeof window.FC_DataLoaded === 'function') {
							FC_DataLoaded(sender.id)
						}
						if (typeof window.FC_Rendered === 'function') {
							FC_Rendered(sender.id)
						}
						if (typeof window.FC_DrawComplete === 'function') {
							FC_DrawComplete(sender.id)
						}
					} catch (err) {
						setTimeout(function() {
							throw err;
				...
				createChart.call(this, b, c)
			} else {
				renderArray.push([ this, a, b ])
			}
		};
		FCC.update = function(a, b) {
			var c = this.id, __containerId = this.options.containerElementId, sender = this, HCJson;
			if (typeof __containerId !== 'undefined'
					&& typeof FCC.items[c] !== 'undefined') {
				if (!b) {
					delete n[c].msgTxt;
					if (this.jsVars) {
				...
				HCJson = D.call(this, __containerId);
				HCJson.chart.events.load = function() {
					J.raiseEvent('loaded', {}, sender);
					J.raiseEvent('dataloaded', {}, sender);
					J.raiseEvent('drawcomplete', {}, sender)
		...
		J.addEventListener('Disposed', function(e) {
			delete n[e.sender.id]
		});
		J.addEventListener('BeforeInitialize', function(e) {
			n[e.sender.id] = new I()
		});
		FCC.config = function(a) {
			J.extend(n[this.id], a)
		};
		J.addEventListener('DataLoadRequested', function(a) {
			var b = a.sender;
			if (b.options.renderer !== 'javascript') {
				return
			}
		...
		J.addEventListener('DataLoadRequestCompleted', function(a, b) {
			var c = a.sender;
		...
		J.addEventListener('DataLoadError', function(a, b) {
			var c = a.sender;
			if (c.options.renderer !== 'javascript') {
				return
			}
	...
	J.renderer.register('javascript', FCC);
	if (/\(iPhone;|\(iPod;|\(iPad;/i.test(navigator.userAgent)) {
		J.renderer.setDefault('javascript')
	}
	J
			.extend({
				'_fallbackJSChartWhenNoFlash' : function() {
					if (!J.swfobject
							.hasFlashPlayerVersion(J.core.options.requiredFlashPlayerVersion)) {
						J.renderer.setDefault('javascript')
					}
				}
			})
...
(function() {
	...
	function quote(b) {
		...
		return escapable.test(b) ? '"'
				+ b.replace(escapable, function(a) {
					var c = meta[a];
...
(function() {
	var h = FusionCharts([ 'private', 'JSON_DataHandler' ]);
	if (h === undefined) {
		return
	}
	var j = (function() {
		...
		}, XML_CHILDNODE = 1, XML_TEXTNODE = 3;
		var f = {
			...
			child : function(a, b, c, d) {
				...
				for (i = 0; i < b.length; i += 1) {
					...
					if (b[i].nodeType === XML_CHILDNODE) {
						childObj = f.attr(b[i].attributes);
						...
						if (b[i].childNodes.length) {
							f.child(childObj, b[i].childNodes, nodeName, d)
						}
						f.append(childObj, a, nodeName)
		...
		var g = function(a) {
			...
			if (a === undefined || a === null
					|| typeof a.toString !== 'function') {
				g.errorObject = new TypeError('xml2json.parse()');
				return b
			}
			...
			b[rootName] = f.attr(root.attributes);
			if (root.childNodes) {
				f.child(b, root.childNodes, rootName, b)
			}
